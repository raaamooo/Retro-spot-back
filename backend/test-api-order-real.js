...
async function main() {
  try {
    ...
    // We already know locations from previous run, let's use the first one
    const locationId = '4010e756-688f-43de-82d7-8754eb23bcd0'; // Table 1
    
    console.log(`Using menuItemId: ${firstItem.id}`);

    const res = await postJson('http://localhost:5000/api/orders', {
      locationId: locationId,
      customerName: 'Test HTTP Real',
      notes: '',
      paymentMethod: 'CASH',
      tipAmount: 0,
