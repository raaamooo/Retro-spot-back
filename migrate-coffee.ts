...
export async function runCoffeeMigration(prisma: PrismaClient) {
  ...
  for (const item of allItems) {
    ...
    let titleCased = words.map(w => {
      if (w.toLowerCase() === 'coca-cola') return 'Coca-Cola';
      if (w.toLowerCase() === 'v60') return 'V60';
      if (w.toLowerCase() === '3-in-1') return '3-in-1';
      if (w.includes('—')) {
      ...
      if (cleanWord.toLowerCase() === 'kitkat') return prefix + 'KitKat' + suffix;
