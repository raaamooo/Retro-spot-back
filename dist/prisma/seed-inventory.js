...
async function main() {
    ...
    const recipe = async (nameEn, ingredientId, qty) => {
        const menuItemId = itemMap[nameEn];
        if (!menuItemId) {
            console.warn(`  ⚠️  Item not found: "${nameEn}"`);
            return;
        }
    ...
    // ── Frappe ───────────────────────────────────────────────────────────────────
    await recipe('Frappe Vanilla', frappeMix.id, 40);
    await recipe('Frappe Vanilla', vanilla.id, 30);
    await recipe('Frappe Vanilla', milk.id, 200);
    await recipe('Frappe Caramel', frappeMix.id, 40);
    await recipe('Frappe Caramel', caramel.id, 30);
    await recipe('Frappe Caramel', milk.id, 200);
    await recipe('Frappe Mocha', frappeMix.id, 40);
    await recipe('Frappe Mocha', mocha.id, 30);
    await recipe('Frappe Mocha', milk.id, 200);
    await recipe('Frappe Nutella', frappeMix.id, 40);
    await recipe('Frappe Nutella', nutella.id, 40);
    await recipe('Frappe Nutella', milk.id, 200);
    await recipe('Frappe Hazelnut', frappeMix.id, 40);
    await recipe('Frappe Hazelnut', hazelnut.id, 30);
    await recipe('Frappe Hazelnut', milk.id, 200);
    await recipe('Frappe Lotus', frappeMix.id, 40);
    await recipe('Frappe Lotus', lotus.id, 40);
    await recipe('Frappe Lotus', milk.id, 200);
    await recipe('Frappe Pistachio', frappeMix.id, 40);
    await recipe('Frappe Pistachio', pistachio.id, 40);
    await recipe('Frappe Pistachio', milk.id, 200);
    await recipe('Frappuchino Classic', frappeMix.id, 40);
    await recipe('Frappuchino Classic', coffeeBeans.id, 18);
    await recipe('Frappuchino Classic', milk.id, 200);
    await recipe('Frappuchino Flavor', frappeMix.id, 40);
    await recipe('Frappuchino Flavor', caramel.id, 30);
    await recipe('Frappuchino Flavor', milk.id, 200);
    // ── Cocktails ─────────────────────────────────────────────────────────────────
    ...
    // ── Waffles ───────────────────────────────────────────────────────────────────
    await recipe('Nutella Waffle', waffleMix.id, 80);
    await recipe('Nutella Waffle', nutella.id, 50);
    await recipe('Lotus Waffle', waffleMix.id, 80);
    await recipe('Lotus Waffle', lotus.id, 50);
    await recipe('Pistachio Waffle', waffleMix.id, 80);
    await recipe('Pistachio Waffle', pistachio.id, 50);
    await recipe('White Chocolate Waffle', waffleMix.id, 80);
    await recipe('White Chocolate Waffle', whitChoc.id, 50);
    await recipe('Waffle Mix Sauce', waffleMix.id, 80);
    await recipe('Waffle Mix Sauce', caramel.id, 20);
    await recipe('Waffle Mix Sauce', nutella.id, 20);
    // ── Yogurt Corner ─────────────────────────────────────────────────────────────
    await recipe('Honey Yogurt', yogurtBase.id, 200);
    await recipe('Honey Yogurt', honey.id, 30);
...
async function ing(nameEn, nameAr, unit, quantityAvailable, lowStockThreshold) {
