...
export default function apiRoutes(io: Server, prisma: PrismaClient) {
  ...
  const inventoryService = new InventoryService(io, notificationService);
  ...
  router.post('/inventory/ingredients/:id/restock', async (req, res) => {
    try {
      ...
      const result = await inventoryService.restockIngredient(
        req.params.id,
  ...
  router.post('/inventory/deduct', async (req, res) => {
    try {
      ...
      const result = await inventoryService.manualDeduction(
        ingredientId, Number(quantityDeducted), reason, adminId
      );
      res.json(result);
    } catch (err) {
  ...
  router.get('/locations/:id/qr', async (req, res) => {
    try {
      ...
      const qrDataUrl = await generateLocationQR(location.name, baseUrl);
